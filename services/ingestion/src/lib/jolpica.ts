...
import { mapOpenF1SessionType, mapCircuitRef } from "./session-utils";
...
export function scheduledSessionToSessionInfo(session: ScheduledSession) {
  return {
    sessionKey: 0,
    sessionType: session.sessionType,
    sessionName: session.sessionName,
    meetingName: session.meetingName,
    circuitName: mapCircuitRef(session.circuitRef) || session.circuitRef,
